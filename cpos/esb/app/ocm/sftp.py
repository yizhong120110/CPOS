...
class SFTP:
    ...
    def searchFileFromSFTPCheckTime( self, pattern ,logger ,seconds = 0  ):
        """ 在SFTP的指定目录下查找文件
    ...
    def download( self, todir, pattern, isBin=False ):
        ...
        for i in range( len(fn) ):
            try:
                ...
                tlog.log_exception("sftp.get异常[%s],[%s]"%(str(flist[i]) , str(fn[i])))
    ...
    def downloadAS( self ,todir ,remotename, localname ):
        ...
        try:
            ...
            tlog.log_exception("downloadAS下载异常")
            #print '下载文件[%s]时出错!' % flist[-1]
            return False
        return True
