...
#日期型字段打包
from cpos.esb.basic.busimodel.fmtstring import e_string , d_string
...
def e_date(value ,  buflen = 0 , align = 'L' , fmt = '%Y%m%d' ,fillchar = ' ' ):
    """
    打包，日期 时间型打包为格式字符串
    参数列表：value:       字段内容
              buflen:      填充后总长度 
              ...
              fillchar:    填充字符(默认空格符)
    """
    return e_datetime( value , buflen , align = align , fmt = fmt , fillchar = fillchar )

def e_time(value , buflen = 0 , align = 'L' , fmt = '%H:%M:%S' ,fillchar = ' ' ):
    """
    打包，日期 时间型打包为格式字符串
    参数列表：value:       字段内容
              buflen:      填充后总长度 
...
def d_date(buf ,  start = 0 , length = 0 ,fmt = '%Y%m%d' , fillchar = ' '):
    """
    解包，获取日期型字段
    参数列表：buf:      原始报文串
              start:    字段起始位置 
              ...
              fillchar: 填充字符(默认空格符)
    """
    t = get_tuple( buf , start , length , fmt , fillchar )
    if t:
        return t.date()
    return None

def d_time(buf ,  start = 0 , length = 0 ,fmt = '%H%M%S' , fillchar = ' '):
    """
    解包，获取时间型字段
    参数列表：buf:      原始报文串
              start:    字段起始位置 
...
def d_datetime(buf ,  start = 0 , length = 0 , fmt = '%Y%m%d%H%M%S' ,fillchar = ' '):
    """
    解包，获取日期时间型字段
    参数列表：buf:      原始报文串
              start:    字段起始位置 
...
import threading
LOCK = threading.Lock()
def get_tuple( buf , start , length , fmt , fillchar ):
    """
    解包  截取指定长度的字符串
    并将得到的字符串转化成日期时间型格式
    """
    with LOCK:
        tmpstr = d_string( buf , start , length, fillchar)
        if tmpstr:
            try:
