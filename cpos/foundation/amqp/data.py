...
def encode_value(pieces, value):
    ...
    if isinstance(value, str):
        ...
        pieces.append(struct.pack('>cI', 'S'.encode(encoding="utf-8"), len(value)))
        pieces.append(value)
        return 5 + len(value)
    elif isinstance(value, bool):
        pieces.append(struct.pack('>cB', 't'.encode(encoding="utf-8"), int(value)))
        return 2
    elif isinstance(value, int):
        pieces.append(struct.pack('>ci', 'I'.encode(encoding="utf-8"), value))
        return 5

    # elif isinstance(value, long):
    #    pieces.append(struct.pack('>cq', 'l'.encode(encoding="utf-8"), value))
    #    return 9
        ...
        if value._exp < 0:
            ...
            pieces.append(struct.pack('>cBi', 'D'.encode(encoding="utf-8"), decimals, raw))
        else:
            # per spec, the "decimals" octet is unsigned (!)
            pieces.append(struct.pack('>cBi', 'D'.encode(encoding="utf-8"), 0, int(value)))
        return 6
    elif isinstance(value, datetime):
        pieces.append(struct.pack('>cQ', 'T'.encode(encoding="utf-8"),
                                  calendar.timegm(value.utctimetuple())))
        return 9
    elif isinstance(value, dict):
        pieces.append(struct.pack('>c', 'F'.encode(encoding="utf-8")))
        ...
        for v in value:
            encode_value(p, v)
        piece = ''.join(p)
        pieces.append(struct.pack('>cI', 'A'.encode(encoding="utf-8"), len(piece)))
        pieces.append(piece)
        return 5 + len(piece)
    elif value is None:
        pieces.append(struct.pack('>c', 'V'.encode(encoding="utf-8")))
        return 1
    else:
