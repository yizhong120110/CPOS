...
class ICMFrame(object):
    ...
    def __init__(self, frame_header=0x01, frame_type=0x01, frame_version=0x0001, token = None,
        ...
        self.token = token or bytes('00000000')
...
def icm_decode_frame (data_in):
    ...
    if data_in[frame_end - 2] !=  'W'  and data_in[frame_end - 1] !=  'S' :
...
def build_icm_frame(commnode_id,message_body,frame_type=0x01,sequance = 0x00000001):
    ...
    icm_frame = ICMFrame(frame_header=0x01, frame_type=frame_type, frame_version=0x0001, 
                            token = bytes('11111111'),sequance = sequance, payload = pl )
    return icm_frame

if __name__ == '__main__':

    fe, df =  icm_decode_frame ( build_icm_frame('nodeid_123','AB中国EFG').marshal()     )  
    print fe
    print df
    print icm_decode_frame_payload( df.payload)[0]
    print icm_decode_frame_payload( df.payload)[1] .decode('utf8')
    print build_icm_frame('ZDFSSVR',b'0053RL9999          {"JYJGM": "1001", "CZGY": "1001"}').marshal()
    print icm_decode_frame(b'\x01\x03\x00\x01\x01\x01\x01\x01\x01\x01\x01\x01\x00\x00\x00\x01\x00\x00\x00"ZDFSSVR\x00\x00\x00\x00\x00\x00\x00\x00\x00Requests accepted.WS')[1].payload
