...
def start():
    ...
    if get_process('nginx', nginx, user=NGINX_USER):
        ...
        #os.system( 'sudo %s/apps/nginx/sbin/nginx -c %s/apps/nginx/conf/nginx.conf'%(os.environ['HOME'] ,os.environ['HOME']) )
        os.system(
    ...
    if get_process('uwsgi', uwsgi, user=UWSGI_USER):
        ...
        os.system('uwsgi -x %s/src/zh_manage/etc/uwsgi.xml' % (os.environ['HOME']))
        print('web服务 启动完毕')
