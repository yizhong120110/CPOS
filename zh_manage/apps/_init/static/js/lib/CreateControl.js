...
var gr_InstallPath = "/download"; //实际项目中应该写从根目录寻址的目录，如gr_InstallPath="/myapp/report/grinstall"; 
...
function dy( url ,cxxx, dyfs ){
    alert('==')
    var rr = sync_post( url , {  'cxxx':cxxx,"nocache":Math.random() } );
    if (rr.ret){
        if ( rr.val =='0' ){
            alert('生成打印文件错误');
        }else{
            var flst = rr.val.split(',')
            CreateReport("Report");
            Report.Register("锐浪报表插件本地测试", "9B028VEE06LCI794PXQ9E2DL31VDS5R0258TSU54TAYQ5BT6S9SEUV4KWPS5");
            Report.LoadFromURL("/files/" + flst[0]);
            Report.LoadDataFromURL("/files/" + flst[1] );
            if( dyfs == '0'){// 打印预览
