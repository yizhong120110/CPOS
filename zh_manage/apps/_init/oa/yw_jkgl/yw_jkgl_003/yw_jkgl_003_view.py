...
from .yw_jkgl_003_service import index_service,data_service,add_service,edit_service,crcs_data_service,crcs_edit_service,del_service,able_service,get_xydz_service,get_crcs_service
...
def index_view():
    ...
    return render_to_string("yw_jkgl/yw_jkgl_003/yw_jkgl_003.html", data)
...
def get_xydz():
    ...
    """
    # 初始化返回值
    data = {'state':False, 'msg':'','xydz':{},'flag':False}
    # 监控对象id
    id = request.forms.id
    try:
...
def get_crcsxx():
    ...
    """
    # 初始化返回值
    data = {'state':False, 'msg':'','csxx':{}}
    # 监控对象id
    id = request.forms.id
    try:
        data = get_crcs_service(id)
    except:
        # 程序异常时，将异常抛出，写入到日志中
        error_msg = traceback.format_exc()
        logger.info(error_msg)
        data['msg'] = '获取传入参数内容失败！异常错误提示信息\n[%s]' % error_msg
    # 将结果反馈给前台
    return data